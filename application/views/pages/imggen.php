...
if (!empty($_POST['name']) && !empty($_POST['id_num']) && !empty($_POST['school'])) { // ensure fields aren't empty
		...
		$tag = imagecreatefromjpeg("assets/gencard.jpg");
		...
		$nameFontSize = 60;
		$nameDimensions = imageftbbox($nameFontSize, 0, 'assets/genBebasNeue Regular.ttf', $name);
		...
		while ($nameWidth > 518) { // while the name exceeds the maximum width, decrement the font size
			...
			$nameDimensions = imageftbbox($nameFontSize, 0, 'assets/genBebasNeue Regular.otf', $name);
			$nameWidth = abs($nameDimensions[4] - $nameDimensions[0]);
		}

		// compute the font size and coordinate of the id number
		$id_numFontSize = 45;
		$id_numDimensions = imageftbbox($id_numFontSize, 0, 'assets/genBebasNeue Regular.otf', $id_num);
		...
		$schoolFontSize = 45;
		$schoolDimensions = imageftbbox($schoolFontSize, 0, 'assets/genBebasNeue Regular.otf', $school);
		...
		imagefttext($tag,$nameFontSize,0,670,276,$blue,'assets/genBebasNeue Regular.otf',$name);

		// put the id_num on the tag
		imagefttext($tag,$id_numFontSize,0,758,387,$blue,'assets/genBebasNeue Regular.otf',$id_num);

		// put the school on the tag
		imagefttext($tag,$schoolFontSize,0,508,562,$blue,'assets/genBebasNeue Regular.otf',$school);
